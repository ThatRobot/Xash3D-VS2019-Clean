...
void CHudSpectator::DrawOverviewEntities()
{
	...
	for (i=0 ; i < MAX_OVERVIEW_ENTITIES ; i++)
	{
		...
		HLSPRITEModel = (struct model_s *)gEngfuncs.GetSpritePointer( m_hsprBeam );
		gEngfuncs.pTriAPI->SpriteTexture( HLSPRITEModel, 0 );
	...
	HLSPRITEModel = (struct model_s *)gEngfuncs.GetSpritePointer( m_hsprCamera );
	gEngfuncs.pTriAPI->RenderMode( kRenderTransAdd );
	gEngfuncs.pTriAPI->SpriteTexture( HLSPRITEModel, 0 );
